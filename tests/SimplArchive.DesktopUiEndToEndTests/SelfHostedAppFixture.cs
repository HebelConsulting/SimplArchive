...
// Boots the REAL application for the desktop-client end-to-end tests: Postgres + SeaweedFS via Testcontainers,
// then the real API launched as a subprocess on a real Kestrel port, so the desktop SimplArchiveApiClient can
// drive it over HTTP exactly as the shipped desktop app does. A demo tenant + admin + sample tree is seeded via
// the app's own Demo:* startup seed (ADR 0214). This is the **desktop** copy of the fixture — identical to the
// web UI fixture but with **no Playwright/Chrome** (the desktop tests never open a browser), so the desktop suite
// stays light and runs as its own parallel CI job (ADR 0378). Keep the two copies in sync.
public sealed class SelfHostedAppFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        // as a WebAuthn RP ID — kept identical to the web fixture so the seeded app behaves the same.
