...
public class DesktopDuplicateDetectionTests
{
    ...
    public async Task Finds_an_identical_document_by_content_hash()
    {
        ...
        await api.UploadFileAsync(repo.Id, fileName, content);
        ...
        var dups = await api.FindDuplicatesAsync(hash);
        ...
        Assert.Empty(await api.FindDuplicatesAsync(Convert.ToHexStringLower(SHA256.HashData(Encoding.UTF8.GetBytes(Guid.NewGuid().ToString())))));
    }
}
