...
public class DesktopNotificationPreferencesTests
{
    ...
    public async Task Read_defaults_mute_a_type_read_back_and_restore()
    {
        ...
        try
        {
            var defaults = await client.GetNotificationPreferencesAsync();
            ...
            var after = await client.GetNotificationPreferencesAsync();
        ...
        finally
        {
            ...
            var restore = (await client.GetNotificationPreferencesAsync()).Select(p => p with { EmailEnabled = true });
            await client.SetNotificationPreferencesAsync(restore);
        }
    }
}
