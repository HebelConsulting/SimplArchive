...
public class DesktopCurrentVersionTests
{
    ...
    public async Task System_fields_carry_the_current_version_number()
    {
        ...
        await api.Documents.UploadFileAsync(folder.Href("children"), docName, Encoding.UTF8.GetBytes("v1"));
        ...
        var v1 = await api.Documents.GetSystemFieldsAsync(doc.Href("versions"));
        Assert.NotNull(v1);
        Assert.Equal(1, v1!.CurrentVersionNumber);

        await api.Documents.UploadNewVersionAsync(doc.Href("versions"), Encoding.UTF8.GetBytes("v2"), ".txt");
        var v2 = await api.Documents.GetSystemFieldsAsync(doc.Href("versions"));
