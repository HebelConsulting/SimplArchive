...
public class DesktopAnnotationTests
{
    ...
    public async Task Note_create_edit_delete_round_trip()
    {
        ...
        await api.Documents.UploadFileAsync(repo.Href("children"), $"noted-{suffix}.txt", Encoding.UTF8.GetBytes("some content"));
        var doc = (await api.Documents.GetChildrenAsync(repo.Href("children"))).First(n => n.Name == $"noted-{suffix}");

        var preview = await api.Documents.GetPreviewAsync(doc.Href("versions"));
        Assert.NotNull(preview.AnnotationsUrl);
        var url = preview.AnnotationsUrl!;

        // Create a note.
        await api.Annotations.CreateAnnotationAsync(url, 0, 0.2, 0.3, "Review this", "#FFEB3B");
        var afterCreate = await api.Annotations.GetAnnotationsAsync(url);
    ...
    public async Task Note_box_carries_a_size_and_can_be_resized()
    {
        ...
        var repo = (await api.Documents.GetRepositoriesAsync())[0];
        await api.Documents.UploadFileAsync(repo.Href("children"), $"sized-{suffix}.txt", Encoding.UTF8.GetBytes("some content"));
        ...
        await api.Annotations.CreateAnnotationAsync(url, 0, 0, 0.2, 0.3, 0.25, 0.1, "Sized note", "#FFEB3B");
        var note = Assert.Single((await api.Annotations.GetAnnotationsAsync(url)).Items);
    ...
    public async Task Multi_select_copy_paste_and_delete()
    {
        ...
        var repo = (await api.Documents.GetRepositoriesAsync())[0];
        await api.Documents.UploadFileAsync(repo.Href("children"), $"multi-{suffix}.txt", Encoding.UTF8.GetBytes("some content"));
        ...
        await api.Annotations.CreateAnnotationAsync(url, 0, 0, 0.5, 0.5, 0.2, 0.06, "note two", "#B3E5FC");
        ...
        var afterPaste = (await api.Annotations.GetAnnotationsAsync(url)).Items;
        ...
        var afterDelete = (await api.Annotations.GetAnnotationsAsync(url)).Items;
    ...
    public async Task Highlight_recolour_move_and_resize()
    {
        ...
        var repo = (await api.Documents.GetRepositoriesAsync())[0];
        await api.Documents.UploadFileAsync(repo.Href("children"), $"hl-{suffix}.txt", Encoding.UTF8.GetBytes("some content"));
        ...
        Assert.Equal("#4FC3F7", Assert.Single((await api.Annotations.GetAnnotationsAsync(url)).Items).Color);
        ...
        var moved = Assert.Single((await api.Annotations.GetAnnotationsAsync(url)).Items);
    ...
    public async Task Shape_tool_resets_after_one_draw()
    {
        ...
        var repo = (await api.Documents.GetRepositoriesAsync())[0];
        await api.Documents.UploadFileAsync(repo.Href("children"), $"tool-{suffix}.txt", Encoding.UTF8.GetBytes("some content"));
    ...
    public async Task Markup_shape_create_read_delete_round_trip()
    {
        ...
        var repo = (await api.Documents.GetRepositoriesAsync())[0];
        await api.Documents.UploadFileAsync(repo.Href("children"), $"markup-{suffix}.txt", Encoding.UTF8.GetBytes("some content"));
