...
// The client half of the inbox page operations (#487, ADR 0575): the reordering arithmetic both dialogs share,
...
public class DesktopInboxPageOperationsTests
{
    ...
    [Theory]
    [InlineData("api/inbox/a.pdf/pages/split", null, true, false)]
    [InlineData(null, "api/inbox/a.pdf/pages/order", false, true)]
    [InlineData("api/inbox/a.pdf/pages/split", "api/inbox/a.pdf/pages/order", true, true)]
    public void Each_page_action_follows_its_own_rel(string? split, string? sort, bool canSplit, bool canSort)
    {
        var actions = new InboxItemActionsViewModel
        {
            Pages = new InboxApi.PagesInfo("pdf", 3, split, sort),
        };
    ...
    [Theory]
    [InlineData(0, "api/inbox/from-items", false)]
    [InlineData(1, "api/inbox/from-items", false)]
    [InlineData(2, "api/inbox/from-items", true)]
    ...
    {
        var actions = new InboxItemActionsViewModel { SelectedCount = selected, JoinHref = joinHref };
    ...
    public async Task Changing_the_selection_clears_the_previous_rows_actions()
    {
        var actions = new InboxItemActionsViewModel
        {
            Pages = new InboxApi.PagesInfo("pdf", 3, "api/inbox/a.pdf/pages/split", "api/inbox/a.pdf/pages/order"),
        };
