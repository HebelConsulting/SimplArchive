...
public class GroupIntrayTests
{
    ...
    public GroupIntrayTests(E2EApiFactory factory) => _factory = factory;

    [Fact]
    public async Task Group_members_share_an_intray_non_members_are_refused()
    {
        ...
        // Alice uploads a scan into the GROUP intray (member-gated).
        var name = $"scan-{Guid.NewGuid():N}.txt";
        var upload = await TestJson.Post(alice, $"/api/intray?group={groupId}", new { fileName = name });
        ...
        // it isn't duplicated across his own + the group intray). Group items are opt-in via ?includeGroups=true
        // (the intray defaults to own-items-only, ADR 0532).
        var groupItem = (await TestJson.Get(bob, "/api/intray?includeGroups=true")).GetProperty("items").EnumerateArray()
        ...
        // Carol (a non-member) sees none of it, and can't upload into or file from the group intray.
        Assert.DoesNotContain(
            (await TestJson.Get(carol, "/api/intray")).GetProperty("items").EnumerateArray().ToList(),
        ...
        Assert.DoesNotContain(
            (await TestJson.Get(bob, "/api/intray?includeGroups=true")).GetProperty("items").EnumerateArray().ToList(),
            i => i.GetProperty("name").GetString() == name);

        // A second file of the now-taken item → 404 (idempotent under contention — two members draining one queue).
        Assert.Equal(HttpStatusCode.NotFound, (await alice.PostAsJsonAsync($"/api/intray/{name}/file?group={groupId}", new { folderId = bobRepo })).StatusCode);
    }
}
