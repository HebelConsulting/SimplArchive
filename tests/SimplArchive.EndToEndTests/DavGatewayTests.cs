...
public class DavGatewayTests
{
    ...
    public async Task WebDav_round_trips_browse_create_read_move_and_delete()
    {
        ...
        var options = await DavAsync("OPTIONS", "/SimplArchive");
        ...
        var rootList = await DavAsync("PROPFIND", "/SimplArchive", headers: [("Depth", "1")]);
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("MKCOL", $"/SimplArchive/{repoName}/wd")).StatusCode);
        var content = Encoding.UTF8.GetBytes("hello webdav");
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/wd/hello.txt", content)).StatusCode);
        ...
        var get = await DavAsync("GET", $"/SimplArchive/{repoName}/wd/hello.txt");
        ...
        var folderXml = await (await DavAsync("PROPFIND", $"/SimplArchive/{repoName}/wd", headers: [("Depth", "1")])).Content.ReadAsStringAsync();
        ...
        var move = await DavAsync("MOVE", $"/SimplArchive/{repoName}/wd/hello.txt", headers: [("Destination", $"/SimplArchive/{repoName}/wd/renamed.txt")]);
        Assert.Equal(HttpStatusCode.Created, move.StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await DavAsync("GET", $"/SimplArchive/{repoName}/wd/renamed.txt")).StatusCode);
        Assert.Equal(HttpStatusCode.NoContent, (await DavAsync("DELETE", $"/SimplArchive/{repoName}/wd/renamed.txt")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await DavAsync("GET", $"/SimplArchive/{repoName}/wd/renamed.txt")).StatusCode);
    }
    ...
    public async Task WebDav_download_temp_stages_then_commits_the_real_file_on_rename(string tempExt)
    {
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("MKCOL", $"/SimplArchive/{repoName}/dl")).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/dl/report.txt", [])).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/dl/report.txt{tempExt}", payload)).StatusCode);
        ...
        var placeholder = await DavAsync("GET", $"/SimplArchive/{repoName}/dl/report.txt");
        ...
        var move = await DavAsync("MOVE", $"/SimplArchive/{repoName}/dl/report.txt{tempExt}",
            headers: [("Destination", $"/SimplArchive/{repoName}/dl/report.txt")]);
        Assert.Equal(HttpStatusCode.Created, move.StatusCode);

        var get = await DavAsync("GET", $"/SimplArchive/{repoName}/dl/report.txt");
        Assert.Equal(HttpStatusCode.OK, get.StatusCode);
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/dl/other.txt{tempExt}", payload)).StatusCode);
        Assert.Equal(HttpStatusCode.NoContent, (await DavAsync("DELETE", $"/SimplArchive/{repoName}/dl/other.txt{tempExt}")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await DavAsync("GET", $"/SimplArchive/{repoName}/dl/other.txt")).StatusCode);
    }
    ...
    public async Task WebDav_clutter_is_filtered_from_the_repo_but_transient_files_stage_in_the_intray()
    {
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/._ghost.txt", bytes)).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/.DS_Store", bytes)).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/Thumbs.db", bytes)).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/download.crdownload", bytes)).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("MKCOL", $"/SimplArchive/{repoName}/.Trashes")).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{repoName}/real.txt", bytes)).StatusCode);
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{Personal}/Intray/.DS_Store", bytes)).StatusCode);
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{Personal}/Intray/partial.crdownload", bytes)).StatusCode);

        var intrayList = await (await DavAsync("PROPFIND", $"/SimplArchive/{Personal}/Intray", headers: [("Depth", "1")])).Content.ReadAsStringAsync();
        Assert.Contains("partial.crdownload", intrayList); // transient is allowed in the staging area

        // The remembered .DS_Store is LISTED to its writer — a listing that omits what a direct GET serves is
        // the coherence defect of #794, and the OS drops its cache of whatever the listing denies. It is still
        // never an Intray ITEM: nothing under the inbox prefix, which is what "discarded" means here.
        Assert.Contains(".DS_Store", intrayList);
    }
    ...
    public async Task WebDav_intray_and_checkout_folders_work()
    {
        ...
        var rootXml = await (await DavAsync("PROPFIND", "/SimplArchive", headers: [("Depth", "1")])).Content.ReadAsStringAsync();
        Assert.Contains(Personal, rootXml);

        // Personal lists the two virtual special folders alongside its real children.
        var personalXml = await (await DavAsync("PROPFIND", $"/SimplArchive/{Personal}", headers: [("Depth", "1")])).Content.ReadAsStringAsync();
        ...
        Assert.Equal(HttpStatusCode.Created, (await DavAsync("PUT", $"/SimplArchive/{Personal}/Intray/staged.txt", Encoding.UTF8.GetBytes("stage me"))).StatusCode);
        Assert.Contains("staged.txt", await (await DavAsync("PROPFIND", $"/SimplArchive/{Personal}/Intray", headers: [("Depth", "1")])).Content.ReadAsStringAsync());
        Assert.Equal("stage me", await (await DavAsync("GET", $"/SimplArchive/{Personal}/Intray/staged.txt")).Content.ReadAsStringAsync());
        Assert.Equal(HttpStatusCode.NoContent, (await DavAsync("DELETE", $"/SimplArchive/{Personal}/Intray/staged.txt")).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await DavAsync("GET", $"/SimplArchive/{Personal}/Intray/staged.txt")).StatusCode);
        ...
        Assert.Contains("codoc.txt", await (await DavAsync("PROPFIND", $"/SimplArchive/{Personal}/Check-out", headers: [("Depth", "1")])).Content.ReadAsStringAsync());
        Assert.Equal("v1", await (await DavAsync("GET", $"/SimplArchive/{Personal}/Check-out/codoc.txt")).Content.ReadAsStringAsync());

        // PUT saves an edited working copy to the stash; GET then returns the stash.
        Assert.Equal(HttpStatusCode.NoContent, (await DavAsync("PUT", $"/SimplArchive/{Personal}/Check-out/codoc.txt", Encoding.UTF8.GetBytes("edited"))).StatusCode);
        Assert.Equal("edited", await (await DavAsync("GET", $"/SimplArchive/{Personal}/Check-out/codoc.txt")).Content.ReadAsStringAsync());
        ...
        const string lockFile = $"/SimplArchive/{Personal}/Check-out/.~lock.codoc.txt%23"; // %23 = '#'
        ...
        Assert.DoesNotContain(".~lock", await (await DavAsync("PROPFIND", $"/SimplArchive/{Personal}/Check-out", headers: [("Depth", "1")])).Content.ReadAsStringAsync());
    ...
    public async Task Wrong_webdav_password_is_refused()
    {
        ...
        using var req = new HttpRequestMessage(new HttpMethod("PROPFIND"), "/SimplArchive");
    ...
    public async Task WebDav_hardening_range_copy_lock_and_quota()
    {
        ...
        Assert.Equal(HttpStatusCode.Created, (await Dav("PUT", $"/SimplArchive/{repo}/data.txt", content)).StatusCode);

        // Range GET → 206 + the sliced bytes.
        var range = await Dav("GET", $"/SimplArchive/{repo}/data.txt", null, ("Range", "bytes=4-7"));
        ...
        Assert.Equal(HttpStatusCode.Created, (await Dav("COPY", $"/SimplArchive/{repo}/data.txt", null, ("Destination", $"/SimplArchive/{repo}/copy.txt"))).StatusCode);
        Assert.Equal("0123456789ABCDEF", await (await Dav("GET", $"/SimplArchive/{repo}/copy.txt")).Content.ReadAsStringAsync());

        // LOCK → an opaque token; UNLOCK with a wrong token → 409, with the real token → 204.
        var lockResp = await Dav("LOCK", $"/SimplArchive/{repo}/data.txt");
        ...
        Assert.Equal(HttpStatusCode.Conflict, (await Dav("UNLOCK", $"/SimplArchive/{repo}/data.txt", null, ("Lock-Token", "<opaquelocktoken:00000000-0000-0000-0000-000000000000>"))).StatusCode);
        Assert.Equal(HttpStatusCode.NoContent, (await Dav("UNLOCK", $"/SimplArchive/{repo}/data.txt", null, ("Lock-Token", $"<{token}>"))).StatusCode);

        // Quota → 507: a tiny quota makes any further PUT exceed it.
        await _factory.SetTenantStorageQuotaAsync(tenantId, 1);
        Assert.Equal(HttpStatusCode.InsufficientStorage, (await Dav("PUT", $"/SimplArchive/{repo}/toobig.txt", content)).StatusCode);
    }
    ...
    public async Task Root_listing_is_acl_filtered()
    {
        ...
        using var req = new HttpRequestMessage(new HttpMethod("PROPFIND"), "/SimplArchive") { Headers = { Authorization = basic } };
