...
// Copying a repository document into the inbox as a TEMPLATE (#467): the bytes and the document's mask + index
...
public class InboxFromDocumentTests
{
    private readonly E2EApiFactory _factory;

    public InboxFromDocumentTests(E2EApiFactory factory) => _factory = factory;

    [Fact]
    public async Task A_document_is_copied_into_the_inbox_with_its_mask_and_index_values()
    {
        ...
        // The action is reached by FOLLOWING the rel the inbox advertises, never by composing the path — the
        // same way a conforming client must (ADR 0543).
        var inbox = await TestJson.Get(owner, "/api/inbox");
        var fromDocument = inbox.GetProperty("links").EnumerateArray()
        ...
        // It lands in the inbox under the document's name plus the version's extension. That naming is not
        // incidental: it is what lets the item later be dragged back onto Check-out and matched by filename.
        var itemName = docName + ".txt";
        var items = (await TestJson.Get(owner, "/api/inbox")).GetProperty("items").EnumerateArray().ToList();
        var item = items.FirstOrDefault(i => i.GetProperty("name").GetString() == itemName);
        Assert.True(item.ValueKind is not System.Text.Json.JsonValueKind.Undefined, $"'{itemName}' is not in the inbox.");
        ...
        var draft = await TestJson.Get(owner, $"/api/inbox/{itemName}/mask");
        ...
        // A second copy of the same document conflicts rather than overwriting: the inbox is addressed BY NAME,
    ...
    public async Task A_folder_has_nothing_to_copy_and_an_unreadable_document_is_refused()
    {
        ...
        var folder = await owner.PostAsJsonAsync("/api/inbox/from-document", new { documentId = folderId });
        ...
        // this, the endpoint would be a way to copy any document in the tenant into your own inbox.
        var refused = await stranger.PostAsJsonAsync("/api/inbox/from-document", new { documentId = folderId });
        ...
        var missing = await owner.PostAsJsonAsync("/api/inbox/from-document", new { documentId = Guid.NewGuid() });
