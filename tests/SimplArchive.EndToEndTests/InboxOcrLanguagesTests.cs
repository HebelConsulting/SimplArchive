...
// End-to-end: staging OCR languages on a scannable inbox item (ADR "Inbox OCR-language staging") is carried by
...
public class InboxOcrLanguagesTests
{
    private readonly E2EApiFactory _factory;

    public InboxOcrLanguagesTests(E2EApiFactory factory) => _factory = factory;

    [Fact]
    public async Task Staged_ocr_languages_are_applied_to_the_filed_version()
    {
        // A real logged-in User (the inbox is scoped to the token's userId).
        var (_, _, tenantId) = await _factory.SeedServiceAccountAsync(canManageRepositories: false);
        var email = $"inboxocr-{Guid.NewGuid():N}@e2e.local";
        const string password = "inboxocr1234";
        await _factory.SeedUserAsync(tenantId, email, password, "Inbox OCR User");
        ...
        (await user.PutAsJsonAsync($"/api/inbox/{name}/mask", new { ocrLanguages = new[] { "deu", "eng" } })).EnsureSuccessStatusCode();

        // File it → the filed version carries the staged, ordered OCR languages.
        var filed = await TestJson.Post(user, $"/api/inbox/{name}/file", new { folderId = repoId });
