...
public class RepositoryAclTests
{
    ...
    public async Task Acl_round_trips_with_permissions_on()
    {
        ...
        {
            result = await new RepositoryImporter(db, storage, accessor, new WellKnownMaskSeeder(db, Microsoft.Extensions.Logging.Abstractions.NullLogger<SimplArchive.Infrastructure.Masks.WellKnownMaskSeeder>.Instance), new SimplArchive.Infrastructure.Storage.StorageQuotaService(db, Microsoft.Extensions.Logging.Abstractions.NullLogger<SimplArchive.Infrastructure.Storage.StorageQuotaService>.Instance), NoOpDocumentIndexQueue.Instance, NoOpSearchablePdfQueue.Instance, new SimplArchive.Api.Documents.PersonalRepositoryProvisioner(db, NoOpAuditRecorder.Instance), new SimplArchive.Api.Documents.PersonalMailboxProvisioner(db, new SimplArchive.Api.Documents.PersonalRepositoryProvisioner(db, NoOpAuditRecorder.Instance)), Microsoft.Extensions.Logging.Abstractions.NullLogger<SimplArchive.Api.Documents.RepositoryImporter>.Instance).ImportAsync(zip, null, updateExisting: false, includePermissions: true, merge: false, SimplArchive.Api.Documents.LeafMergeMode.Rename, CancellationToken.None);
        }
    ...
    public async Task Import_ignores_acl_when_permissions_off()
    {
        ...
        using (var db = Ctx(connection, accessor))
        {
            var result = await new RepositoryImporter(db, storage, accessor, new WellKnownMaskSeeder(db, Microsoft.Extensions.Logging.Abstractions.NullLogger<SimplArchive.Infrastructure.Masks.WellKnownMaskSeeder>.Instance), new SimplArchive.Infrastructure.Storage.StorageQuotaService(db, Microsoft.Extensions.Logging.Abstractions.NullLogger<SimplArchive.Infrastructure.Storage.StorageQuotaService>.Instance), NoOpDocumentIndexQueue.Instance, NoOpSearchablePdfQueue.Instance, new SimplArchive.Api.Documents.PersonalRepositoryProvisioner(db, NoOpAuditRecorder.Instance), new SimplArchive.Api.Documents.PersonalMailboxProvisioner(db, new SimplArchive.Api.Documents.PersonalRepositoryProvisioner(db, NoOpAuditRecorder.Instance)), Microsoft.Extensions.Logging.Abstractions.NullLogger<SimplArchive.Api.Documents.RepositoryImporter>.Instance).ImportAsync(zip, null, updateExisting: false, includePermissions: false, merge: false, SimplArchive.Api.Documents.LeafMergeMode.Rename, CancellationToken.None);
