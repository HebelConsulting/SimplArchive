...
public class WebWebDavTests
{
    ...
    public async Task WebDav_dialog_shows_the_mount_url_and_generates_a_password()
    {
        ...
        // The dialog exposes the Personal folder URL field (Inbox / Check-out / archive live under it).
        await Expect(dialog.GetByRole(AriaRole.Textbox, new() { Name = "Personal folder (Inbox, Check-out, archive)" })).ToBeVisibleAsync();
    ...
    public async Task Inbox_tab_copies_the_personal_webdav_url()
    {
        ...
        // Inbox tab → Copy WebDAV URL → the Personal mount URL lands on the clipboard.
        ...
        Assert.EndsWith("/webdav/Personal", await page.EvaluateAsync<string>("() => navigator.clipboard.readText()"));
    }
}
