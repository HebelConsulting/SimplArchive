...
// A UI flow (ADR 0532): the demo admin (a CanManageInboxes holder) uploads an item to their own inbox, hands it
// to another user via the "Send to…" dialog — it leaves the admin's inbox — and then, using the admin user-picker,
// opens that user's inbox and sees the item there.
[Collection(UiCollection.Name)]
[Trait("Area", "ui-2")]
public class WebInboxSendTests
{
    private readonly SelfHostedAppFixture _app;

    public WebInboxSendTests(SelfHostedAppFixture app) => _app = app;
    ...
    public async Task Send_an_own_item_to_another_user_then_triage_it_via_the_user_picker()
    {
        ...
        // Inbox: upload a file into the admin's own inbox.
        ...
        await page.SetInputFilesAsync("#inbox-file-input", new FilePayload
        {
            Name = name + ".txt",
            MimeType = "text/plain",
            Buffer = Encoding.UTF8.GetBytes("hand-off via the inbox"),
        });
        ...
        // It leaves the admin's own inbox...
