...
// The web intray file-list drop-zone (ADR "Inbox file-list drop-zone"): dropping OS files onto the intray list
// uploads them straight into the S3-backed intray. Playwright can't perform a real OS file drag, so the drop is
// synthesized with a DataTransfer holding a File; the JS handler presigns + PUTs it exactly like a real drop.
[Collection(UiCollection.Name)]
[Trait("Area", "ui-2")]
public class WebIntrayDropTests
{
    private readonly SelfHostedAppFixture _app;

    public WebIntrayDropTests(SelfHostedAppFixture app) => _app = app;

    [Fact]
    public async Task Dropping_a_file_on_the_intray_list_uploads_it()
    {
        var page = await Ui.LoginAsync(_app);
        var name = "webdrop-" + Guid.NewGuid().ToString("N")[..8];

        await page.Locator(".wb-tab[aria-label=\"Intray\"]").First.ClickAsync();
        var zone = page.Locator(".wb-intray-drop");
        ...
        // The dropped file uploads to the intray and appears in the list. The upload is confirmed 200, but the
