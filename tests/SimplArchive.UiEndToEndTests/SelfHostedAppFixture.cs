...
// Boots the REAL application for browser-driven end-to-end tests: Postgres + MinIO via Testcontainers, then the
// real API launched as a subprocess on a real Kestrel port (a browser can't reach WebApplicationFactory's
// in-memory TestServer), serving the Blazor WASM client. A demo tenant + admin + sample tree is seeded via the
// app's own Demo:* startup seed (ADR 0214), so the browser logs straight in and has content. One Chrome
// (system channel — no browser download) is shared across the collection.
public sealed class SelfHostedAppFixture : IAsyncLifetime
{
    ...
    // used by the search-refinement test.
    ...
    private readonly IContainer _tika = new ContainerBuilder()
        ...
        // Cap the Tika JVM heap so the fleet fits a memory-constrained (≈16 GB) runner (same rationale as
        // E2EApiFactory). JAVA_TOOL_OPTIONS, not JAVA_OPTS — the image entrypoint runs `exec java …` and never
        // references JAVA_OPTS, whereas the JVM auto-reads JAVA_TOOL_OPTIONS at startup.
    ...
    // Gotenberg — office/markdown/html → PDF renditions (for the converted-preview + preview-find tests).
    ...
    // The self-hosted app's Postgres — exposed so a test can clean up data it seeded (e.g. removing a passkey
    // from the shared demo admin so it doesn't affect other tests' logins).
    ...
    private async Task StartApiAsync(string storageUrl)
    {
        ...
        var env = new Dictionary<string, string>
        {
            ...
            // Seed a demo tenant + admin (known password) + sample tree so the browser has data to work with.
            ["Demo__Tenant__Name"] = "Demo",
            ["Demo__Administrator__Email"] = AdminEmail,
            ["Demo__Administrator__Password"] = AdminPassword,
            ["Demo__Administrator__DisplayName"] = "Demo Admin",
