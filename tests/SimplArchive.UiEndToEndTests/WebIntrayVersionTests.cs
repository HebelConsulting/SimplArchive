...
// A UI flow (ADRs 0286/0287/0528): with a document selected on Repositories, the intray filing dialog can file an
// item as a NEW VERSION of it — the item leaves the intray, and the filing comment is now the new version's comment
// (shown in the versions dialog), no longer a chat/feed post.
[Collection(UiCollection.Name)]
[Trait("Area", "ui-3")]
public class WebIntrayVersionTests
{
    private readonly SelfHostedAppFixture _app;

    public WebIntrayVersionTests(SelfHostedAppFixture app) => _app = app;

    [Fact]
    public async Task Filing_an_intray_item_as_a_new_version_writes_the_version_comment_not_a_feed_post()
    {
        var page = await Ui.LoginAsync(_app);
        var doc = "verdoc-" + Guid.NewGuid().ToString("N")[..8];
        var intrayItem = "newver-" + Guid.NewGuid().ToString("N")[..8];
        ...
        // Upload an intray item and file it as a new version of the selected document.
        await page.Locator(".wb-tab[aria-label=\"Intray\"]").First.ClickAsync();
        await page.SetInputFilesAsync("#intray-file-input", new FilePayload { Name = intrayItem + ".txt", MimeType = "text/plain", Buffer = Encoding.UTF8.GetBytes("v2") });

        await page.Locator(".wb-list-row").Filter(new() { HasText = intrayItem }).Locator("button").Last.ClickAsync();
        ...
        var commentText = "checked-in via intray " + Guid.NewGuid().ToString("N")[..6];
        ...
        await Expect(page.Locator(".wb-list-row").Filter(new() { HasText = intrayItem })).Not.ToBeVisibleAsync();
