...
// A UI flow (ADR 0290): multi-select 2+ intray items and "File N items" into a picked folder via the bulk
// filing dialog — all selected items leave the intray and appear as documents in that folder.
[Collection(UiCollection.Name)]
[Trait("Area", "ui-2")]
public class WebIntrayBulkFilingTests
{
    private readonly SelfHostedAppFixture _app;

    public WebIntrayBulkFilingTests(SelfHostedAppFixture app) => _app = app;

    [Fact]
    public async Task Bulk_files_multiple_intray_items_into_a_folder()
    {
        ...
        await page.SetInputFilesAsync("#intray-file-input", new[]
        {
