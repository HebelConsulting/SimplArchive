...
// A UI flow (ADR 0291): an un-classified intray item shows in square brackets; staging a mask writes the
// {name}.mask.json sidecar and flips the item to un-bracketed.
[Collection(UiCollection.Name)]
[Trait("Area", "ui-2")]
public class WebIntrayMaskTests
{
    private readonly SelfHostedAppFixture _app;

    public WebIntrayMaskTests(SelfHostedAppFixture app) => _app = app;
    ...
    public async Task Staging_a_mask_flips_the_unclassified_indicator()
    {
        ...
        // Upload to the intray → the un-classified item shows in brackets.
        await page.Locator(".wb-tab[aria-label=\"Intray\"]").First.ClickAsync();
        await page.SetInputFilesAsync("#intray-file-input", new FilePayload { Name = file, MimeType = "text/plain", Buffer = Encoding.UTF8.GetBytes("mask me") });
