...
/// <remarks>
/// <para>
/// Three surfaces need the same list: the Repositories detail pane (per-version languages), the Inbox staging
/// form (staged before filing), and the Tenant tab (the tenant-wide default). While the workbench was one page
/// they shared a field. The Inbox extraction gave that tab its own lazily-loaded copy — defensible for one — and
...
/// </para>
/// <para>
/// Fetched on first use rather than at startup: a session that never opens a scannable document, the inbox, or
