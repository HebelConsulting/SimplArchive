...
/// <summary>Starting an upload into the caller's intray: one presigned PUT per file.</summary>
/// <remarks>
/// Extracted at the SECOND caller, which is where CLAUDE.md says to stop copying. The Intray tab has staged
/// uploads this way since the tab existed; dropping files onto the Personal ▸ Intray tree node (#467) is the
...
/// </remarks>
public sealed class IntrayUploads(HttpClient http, ApiRoot apiRoot)
{
    ...
    /// <summary>
    /// A presigned PUT for one file, or <c>null</c> when the intray refused it. Null rather than an exception
    ...
    public async Task<string?> CreateTargetAsync(string fileName)
    {
        try
        {
            var response = await http.PostAsJsonAsync(await apiRoot.RequireAsync("intray"), new { fileName });
            var target = await response.Content.ReadFromJsonAsync<UploadIntrayResponse>();
    ...
    /// <remarks>
    /// <para>
    /// This is the "client-signalled path" <c>IntrayIngestSweepWorker</c>'s own comment describes — and it was
