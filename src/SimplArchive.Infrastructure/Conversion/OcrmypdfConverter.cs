...
public sealed class OcrmypdfConverter : ISearchablePdfConverter
{
    ...
    public async Task<byte[]?> ConvertToSearchablePdfAsync(byte[] sourceBytes, SearchablePdfSourceKind kind, string languages, bool deskew = false, bool rotate = false, bool force = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var url = $"ocr?lang={Uri.EscapeDataString(languages)}&kind={kindParam}{(deskew ? "&deskew=true" : string.Empty)}{(rotate ? "&rotate=true" : string.Empty)}{(force ? "&force=true" : string.Empty)}";
...
{
    public Task<byte[]?> ConvertToSearchablePdfAsync(byte[] sourceBytes, SearchablePdfSourceKind kind, string languages, bool deskew = false, bool rotate = false, bool force = false, CancellationToken cancellationToken = default) =>
