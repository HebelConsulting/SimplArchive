...
public sealed class PatchCodeIngestProcessor(
    ...
    IPatchCodeDetector detector) : IIntrayIngestProcessor
{
    ...
    public async Task<IntrayProcessed?> TryProcessAsync(IntrayIngestContext context, CancellationToken cancellationToken)
    {
        ...
        var wanted = await dbContext.Users
            ...
            .Select(u => (bool?)u.CutIntrayUploadsAtPatchCodes)
        ...
        return new IntrayProcessed(parts.Select(p => new IntrayPart(p, extension, contentType)).ToList());
    }
}
