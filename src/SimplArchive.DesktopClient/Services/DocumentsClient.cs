...
public sealed partial class DocumentsClient(ApiCore core, Func<RemindersClient> reminders)
{
    ...
    // (#426, ADR 0543).
    // GrantableRights: what this caller may confer here (#877, ADR 0726) — the EffectiveRights.Covers cap the
    // server enforces with a 403 and never advertised. Null (=> nothing offered) when the server omits it.
