...
public sealed class CalendarContactClassifier
{
    ...
    // collaborator, reached from nowhere else, and a DI registration would advertise a second entrance to a
    // door that must stay single.
    private readonly ResourceCollectionWriter _resources;
        ...
        Document document, DocumentVersion version, string content, Guid maskId, Guid maskVersionId, CancellationToken cancellationToken)
    {
        ...
        ResourceCollectionWriter.PendingBookingAudit? pendingAudit = null;
        var values = new List<(string Field, string? Value)>
        {
        ...
        if (maskId == WellKnownMaskIds.Booking)
        {
            ...
            pendingAudit = await _resources.UpsertBookingRowAsync(document, version, occurrence, cancellationToken);
        }
        else if (maskId == WellKnownMaskIds.AvailabilityWindow)
        {
            // The window IS the .ics, like its two siblings (ADRs 0744/0778/0780): the same pass that indexes
            // the fields writes the row, before ApplyAsync so both ride the save the invariants judge.
    ...
    /// shows.</remarks>
    private static DateTimeOffset? Instant(Ical.Net.DataTypes.CalDateTime? when) => CalendarInstants.Instant(when);
