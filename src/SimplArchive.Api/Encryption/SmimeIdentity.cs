...
public static class SmimeIdentity
{
    ...
    public static Generated Generate(string email, string pkcs12Password, TimeSpan lifetime)
    {
        ...
        using var certificate = request.CreateSelfSigned(notBefore, notBefore + lifetime);

        var pkcs12 = certificate.Export(X509ContentType.Pkcs12, pkcs12Password);
        return new Generated(
            certificate.ExportCertificatePem(),
            pkcs12,
            BuildMobileConfig(email, pkcs12, pkcs12Password));
    }
    ...
    /// </summary>
    private static string BuildMobileConfig(string email, byte[] pkcs12, string pkcs12Password)
    {
