...
/// <summary>
/// The intray's page operations — split one staged scan into its pages, join several into one, and sort the
/// pages of one without splitting it (issue #487).
/// </summary>
/// <remarks>
/// <para>
/// A scanner does not know where one document ends and the next begins: it produces a stack. The intray is
...
/// everything about the intray: which object key a name maps to, what the results are called, that a staged
...
/// </para>
/// </remarks>
public sealed class IntrayPageService(
    ...
    IPatchCodeDetector patchCodes)
{
    ...
    /// <summary>
    /// One new intray item per page, named "&lt;stem&gt; (n)". The source is left alone.
    /// </summary>
    /// <remarks>
    ...
    public async Task<IReadOnlyList<string>> SplitAsync(string prefix, string name, CancellationToken cancellationToken)
    {
        ...
        if (pages.Count < 2)
        {
            throw new IntrayItemHasNoPagesToSplitException(name);
        }
    ...
    /// <summary>
    /// One new intray item holding every page of the named items, in the order given. The sources are left alone.
    /// </summary>
    /// <remarks>
        ...
        CancellationToken cancellationToken)
    {
        if (names.Count < 2)
        {
            throw new IntrayJoinNeedsSeveralItemsException();
        }
        ...
        {
            throw new IntrayJoinNotHomogeneousException();
        }
        ...
        foreach (var name in names)
        {
            ...
            {
                throw new IntrayItemIsSignedException(name);
            }

            if (PageComposer.CountPages(bytes, format) == 0)
            {
                throw new IntrayItemHasNoPagesException(name);
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new IntrayPageOrderInvalidException(name, pageCount);
        }
    ...
    public async Task<string?> DeskewAsync(string prefix, string name, CancellationToken cancellationToken)
    {
        ...
        {
            throw new IntrayPagesNotSupportedException(name);
        }
    ...
    /// <remarks>
    /// <para>
    /// <b>The source is kept, renamed</b> with <see cref="IntrayNaming.ToBeDeletedSuffix"/> rather than deleted.
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new IntrayNoPatchCodesFoundException(name);
        }

        var parts = PageComposer.CutAt(bytes, format, separators);
        if (parts.Count == 0)
        {
            throw new IntrayNoPatchCodesFoundException(name); // nothing but separator sheets: no document in there
        }
        ...
        var kept = await FreeNameAsync(prefix, $"{stem}{IntrayNaming.ToBeDeletedSuffix}{extension}", cancellationToken);
        await storage.CopyObjectAsync(prefix + name, prefix + kept, cancellationToken);
        await IntrayNaming.MoveMaskDraftAsync(storage, prefix, name, kept, cancellationToken);
        await storage.DeleteObjectAsync(prefix + name, cancellationToken);
    ...
    // and nothing in the intray has been filed yet, so there is nothing more specific to ask for.
    ...
    {
        await using var stream = await storage.GetObjectForClientAsync(key, "the intray page view", cancellationToken);
    ...
    private Task CopyMaskDraftAsync(
        ...
        string targetName,
        CancellationToken cancellationToken) =>
        IntrayNaming.CopyMaskDraftAsync(storage, prefix, sourceName, targetName, cancellationToken);
    ...
    private async Task<string> FreeNameAsync(string prefix, string candidate, CancellationToken cancellationToken) =>
        await IntrayNaming.FreeNameAsync(storage, prefix, candidate, cancellationToken)
        ?? throw new IntrayItemNameConflictException(candidate);
    ...
    private static string Stem(string name) => IntrayNaming.Stem(name);


    /// <summary>The staged-mask sidecar's name suffix, as the Api's controllers already know it.</summary>
    /// <remarks>
    /// Forwards to <see cref="IntrayNaming"/>, which is where the intray's naming now lives: the ingest pipeline
    /// in Infrastructure needs the same constant, and Infrastructure cannot see the Api.
    /// </remarks>
    public const string MaskSidecarSuffix = IntrayNaming.MaskSidecarSuffix;
}
