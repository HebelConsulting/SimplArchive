...
using (var scope = app.Services.CreateScope())
{
    ...
    // Env-driven idempotent demo-data seed (Docker Compose only) — provisions a demo tenant plus a
    // TenantAdministrator with a KNOWN password and a small sample tree (a folder and a document with a mask
    // + index-data), so you can browse straight to the UI, log in, and actually see content. Gated on the
    // demo config being present AND the tenant not already existing, so it's a no-op on restart and in every
    // environment that doesn't configure it (default appsettings ships the keys empty). See ADR "Compose
    // demo-data seeding".
    var demoTenantName = app.Configuration["Demo:Tenant:Name"];
    var demoAdminEmail = app.Configuration["Demo:Administrator:Email"];
    var demoAdminPassword = app.Configuration["Demo:Administrator:Password"];
        ...
        && !string.IsNullOrWhiteSpace(demoAdminPassword))
    {
        ...
        if (!await dbContext.Tenants.AnyAsync(t => t.Name == demoTenantName && t.Status == TenantStatus.Active))
        {
            var provisioned = await services.GetRequiredService<ITenantProvisioningService>().ProvisionAsync(
                demoTenantName,
                demoAdminEmail,
                app.Configuration["Demo:Administrator:DisplayName"] ?? "Demo Admin",
                app.Configuration["Demo:RepositoryName"],
                demoAdminPassword);

            // The sample tree reads masks/field definitions and relies on SaveChanges' required-field
            // validation, all of which are tenant-filtered — so set the tenant on the accessor the DbContext
            // reads from (nothing did, since this isn't a request). Provisioning above runs before this with
            // no tenant set, exactly like the platform-admin onboarding path.
            services.GetRequiredService<CurrentTenantAccessor>().TenantId = provisioned.TenantId;

            // Give the demo tenant a 250 MB storage quota (ADR "Per-tenant storage quota") — production tenants
            // are provisioned with no quota (unlimited). Showcases the quota UI + enforcement from the single login.
            var demoTenant = await dbContext.Tenants.SingleAsync(t => t.Id == provisioned.TenantId);
            ...
            // Assign the "Basic Entry" well-known mask (document type). It no longer has a required field
            // (Short Description was Document.Name; Doc Date is now DocumentVersion.DocumentDate — ADR "Drop
            // redundant Short Description / Doc Date mask fields"), so no index data needs filling first.
            var basicEntryVersion = await dbContext.MaskVersions
                .SingleAsync(v => v.MaskId == WellKnownMaskIds.BasicEntry && v.IsCurrent);

            // Give the mask type a review SLA (ADR "Workflow escalation / SLA reminders") so the demo review
            // has a deadline the Tasks tab can show, and a retention period (ADR "Retention policies
            // (auto-disposition)") so the demo document appears on the Retention schedule.
            basicEntryVersion.ReviewSlaDays = 7;
            basicEntryVersion.RetentionYears = 7;
            document.MaskVersionId = basicEntryVersion.Id;
            await dbContext.SaveChangesAsync();

            // Give the document a confirmed version and put it into the approval workflow (ADR "Workflow engine
            // slice 1"), so the demo shows the feature live: the admin's Tasks tab has a pending review and the
            // document's workflow panel offers Approve/Release. The admin is the assigned reviewer (self-review)
            ...
            var version = new DocumentVersion
            {
                ...
                VersionNumber = 1,
                Sha256Hash = Convert.ToHexStringLower(SHA256.HashData(fileBytes)),
                ObjectKey = objectKey,
                CreatedByUserId = provisioned.AdministratorId,
            ...
            var workflowState = new WorkflowState
            {
                ...
                AssignedToUserId = provisioned.AdministratorId,
                CreatedAt = now,
                UpdatedAt = now,
